...
class System:
    ...
    def GetCode(self):
        response = requests.get(f"{self.url}/CheckCode.aspx")
    ...
    def Login(self,username,password):
        ...
        if response.status_code == 302 and "/xs_main.aspx?xh=" in response.headers["Location"]:
            ...
            if "验证码不正确" in html:
                return False,"系统繁忙" # CODE INCORRECT
            ...
            if "密码错误" in html:
                ...
                try:
                    limitLoginCount = findall(r'您还有(.*?)次尝试机会', html)[0]
                except:
                    return False,f"账号锁定"
    ...
    def GetScore(self,stuYear,stuSem):
        ...
        response = requests.get(f"{self.url}/xscj.aspx?xh={self.stuId}",headers=headers,allow_redirects=False)
        html = trimContent(toUtf8(response.text))
        ...
        try:
            ...
            response = requests.post(f"{self.url}/xscj.aspx?xh={self.stuId}",headers=headers,data=data,allow_redirects=False)
    ...
    def GetCourses(self):
        ...
        try:
            ...
            response = requests.get(f"{self.url}//xskbcx.aspx?xh={self.stuId}",headers=headers,allow_redirects=False)
